...
| 3. Envelope na Clicksign (draft → docs → signers → requirements → running) | — | pendente |
...
## Etapas 3–5 (planejado)

- **Etapa 3**: sequência JSON:API no sandbox — `POST /envelopes` (draft) →
  `POST /envelopes/:id/documents` → signers (cliente: widget embedded, grupo 1;
  Icomm: `auto_signature`, grupo 2) → requirements → `PATCH` envelope para
  `running`. Nomes exatos dos requisitos de autenticação serão confirmados na
  referência oficial (developers.clicksign.com) antes de codar.
- **Etapa 4**: endpoint que devolve a chave de assinatura embedded do
  signatário cliente + widget da Clicksign na página pós-pagamento.
